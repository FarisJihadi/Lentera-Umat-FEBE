...
import uploadFileToCloudinary from "../../utils/uploadbook";
...
export default function FormUploadMateri({ onUploadSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      Swal.fire(
        "Peringatan!",
        "Harap lengkapi semua data, unggah file PDF, dan unggah cover materi.",
        "warning"
      );
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="Contoh: Panduan Berwirausaha Islami"
          />
        </div>
        <div>
        ...
        <div>
          ...
          <label
            htmlFor="upload-pdf"
            className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-blue-400 transition"
          >
            {pdfFilePreview ? (
              <div className="flex flex-col items-center">
                <p className="text-xl mb-2">📄</p>
                <p className="text-sm font-medium text-gray-700">
                  File terpilih: {pdfFilePreview.name}
                </p>
              </div>
            ) : (
              <>
              ...
              <div className="mt-4 text-sm text-gray-700 w-full">
                <div className="w-full flex justify-center">
                  <img
                    ...
                    alt="Cover Preview"
                    className="w-full max-h-[150px] object-contain rounded border"
                  />
                </div>
                <p className="mt-2 text-gray-600 text-center text-sm">
                  Nama file: {coverFilePreview.name}
                </p>
              </div>
            ) : (
              <>
        ...
        <div className="flex justify-end">
          <button
            ...
            className="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-6 py-3 rounded-full flex gap-2 items-center disabled:opacity-50"
          >
