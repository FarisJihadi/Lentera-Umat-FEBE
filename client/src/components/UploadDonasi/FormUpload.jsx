...
export default function FormUpload() {
  ...
  const handleChange = (e) => {
    ...
    if (name === "provinsi") {
      const selectedProvince = Indonesia.find(
        (provinsi) => provinsi.namaProvinsi === value
      );
      setKabupatenOptions(
        selectedProvince ? selectedProvince.kabupatenKota : []
      );
      setFormData((prevData) => ({
        ...prevData,
        kabupaten: "",
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      Swal.fire(
        "Peringatan!",
        "Harap lengkapi semua informasi sebelum mengirim donasi",
        "warning"
      );
      return;
    }
  ...
  return (
    <div className="max-w-3xl mx-auto px-8 sm:px-6 md:px-16 py-10 sm:py-16 mb-16 rounded-xl sm:rounded-2xl shadow-md sm:shadow-[0px_0px_6px_2px_rgba(0,0,0,0.12)]">
      <h2 className="text-xl sm:text-2xl text-center text-primary font-semibold mb-8 sm:mb-12">
        Silakan Isi Form Donasi Barang secara Detail
      </h2>
      <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {formData.fotoBarang?.type.startsWith("image/") ? (
                    <div className="w-full flex justify-center">
                      <img
                        src={filePreview.url}
                        alt="Preview"
                        className="w-full max-h-[200px] object-cover rounded border"
                      />
                    </div>
                  ) : (
                    <p className="italic">
                      Tidak dapat menampilkan pratinjau untuk file ini
                    </p>
                  )}
                  <p className="mt-2 text-gray-600 text-center text-sm">
                    Nama file: {filePreview.name}
                  </p>
                </div>
              ) : (
                <>
        ...
        <div className="relative">
          ...
          <div
            ...
            className="w-full p-4 pr-10 border-2 border-gray-300 rounded-lg outline-none text-sm sm:text-base cursor-pointer"
          >
          ...
          {dropdowns.kategori && (
            <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-lg max-h-60 overflow-auto shadow-lg animate-fadeIn">
              {["Pendidikan", "Disabilitas", "Elektronik"].map((item) => (
                <div
                  key={item}
                  onClick={() => selectOption("kategori", item)}
        ...
        <div className="relative">
          ...
          <div
            onClick={() => toggleDropdown("kondisiBarang")}
            className="w-full p-4 pr-10 border-2 border-gray-300 rounded-lg outline-none text-sm sm:text-base cursor-pointer"
          >
          ...
          {dropdowns.kondisiBarang && (
            <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-lg max-h-60 overflow-auto shadow-lg animate-fadeIn">
              {["Baru", "Layak pakai"].map((item) => (
                <div
                  key={item}
                  onClick={() => selectOption("kondisiBarang", item)}
                  className="px-4 py-2 hover:bg-gray-100 cursor-pointer"
                >
                  {item}
                </div>
        ...
        <div className="relative">
          ...
          <div
            ...
            className="w-full p-4 border-2 border-gray-300 rounded-lg outline-none text-sm sm:text-base cursor-pointer"
          >
            ...
            {dropdowns.kabupaten && (
              <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-lg max-h-60 overflow-auto shadow-lg animate-fadeIn">
                {kabupatenOptions.map((kabupaten) => (
                  <div
                    key={kabupaten}
                    onClick={() => selectOption("kabupaten", kabupaten)}
                    className="px-4 py-2 hover:bg-gray-100 cursor-pointer"
                  >
                    {kabupaten}
                  </div>
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            className="bg-primary hover:bg-primary-700 flex gap-2 text-white font-semibold px-6 py-3 rounded-full shadow-md transition text-sm sm:text-base"
          >
            <p>Kirim</p>
