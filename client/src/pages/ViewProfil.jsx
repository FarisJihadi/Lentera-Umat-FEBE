...
const ViewProfil = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const [userRes, detailRes] = await Promise.all([
          axiosInstance.get(`/user/get/${id}`),
          axiosInstance.get(`/detil/get/${id}`),
        ]);
    ...
    const fetchArtikel = async () => {
      try {
        ...
        const artikelFiltered = res.data.filter(
          (artikel) => artikel.artikelUid === id
        );
  ...
  const hasValidProfilePhoto =
    detilUser?.fotoProfil && detilUser.fotoProfil !== "undefined";

  if (loading || !user || !detilUser || artikelList === null) {
    return (
      <div className="text-center mt-20 text-md h-screen">
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto pt-4 px-8 flex md:flex-row flex-col justify-between">
        <div className="flex gap-4 md:gap-8">
          <img
            src={hasValidProfilePhoto ? detilUser.fotoProfil : personProfile}
            alt="profile"
            className="w-24 h-24 md:w-36 md:h-36 bg-gray-500  relative md:-top-16 -top-10 border-4 border-white rounded-md object-cover"
          />
          <div>
            <h1 className="-mt-2 md:text-xl text-base font-semibold flex items-center">
              {detilUser?.namaLengkap || "Nama tidak tersedia"}
            </h1>
            <p className="text-gray-600 md:text-base text-[14px]">
            ...
            <div className="flex mt-3 gap-2 text-lg text-gray-700">
              ...
              {detilUser?.noWa && (
                <a
                  href={`https://wa.me/${detilUser.noWa}`}
                  target="_blank"
                  rel="noopener noreferrer"
      ...
      <div className="max-w-5xl mx-auto px-8 -mt-6 ">
        <p>
          Bio:{" "}
          {detilUser?.bio || "Belum ada bio ditambahkan oleh pengguna ini."}
        </p>
      </div>
            ...
            {artikelList.length > 0 ? (
              artikelList.map((artikel) => (
                <CardArtikel
                  ...
                  date={new Date(artikel.createdAt).toLocaleDateString(
                    "id-ID",
                    {
                      day: "numeric",
                      month: "long",
                  ...
                  handleClick={() =>
                    navigate(`/artikel/detail-artikel/${artikel._id}`)
                  }
                />
              ))
            ) : (
