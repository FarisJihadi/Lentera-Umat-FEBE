...
export default function DetailDonasiSaya() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const resDetilDonasi = await axiosInstance.get(
          `/donasi/detil/get/${path}`
        );
          ...
          const komunitasData = await Promise.all(
            resDetilDonasi.data.permohonan.map(async (permohonan) => {
              const res = await axiosInstance.get(
                `/detil/get/${permohonan.pemohonId}`
              );
              return res.data;
            })
          );
  ...
  if (error)
    return (
      <div className="text-center mt-20 text-md h-screen text-red-500">
        {error}
      </div>
    );
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <span className="bg-blue-100 text-blue-800 text-xs font-semibold px-3 py-1 rounded-full">
                    {detilDonasi?.permohonan?.length || 0} Permintaan
                  </span>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-xs">
                      {donasi.kabupaten + ", " + donasi.provinsi}
                    </p>
                  </div>
                </div>
                <div className="flex items-center space-x-3">
