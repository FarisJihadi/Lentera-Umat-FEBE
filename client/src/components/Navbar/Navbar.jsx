...
function Navbar({
  ...
  navLinkClass = (isActive) =>
    `ml-4 ${isActive ? "text-primary font-bold" : ""} hover:text-primary`,
          ...
          <div className="w-full flex-col flex 940:flex-row py-16 940:py-0 text-center justify-center gap-4 md:gap-10">
            ...
            <Link
              ...
              className={
                menuOpen
                  ? "text-black"
                  : "text-white" + navLinkClass(isLinkActive(paths.book))
              }
            >
              Ummah Book
            </Link>
            <Link
              ...
              className={
                menuOpen
                  ? "text-black"
                  : "text-white" + navLinkClass(isLinkActive(paths.partner))
              }
            >
              Ummah Partner
            </Link>
            {user?.role === "komunitas" ? (
              ...
              <Link
                ...
                className={
                  menuOpen
                    ? "text-black"
                    : "text-white" + navLinkClass(isLinkActive(paths.upload))
                }
              >
                Upload Donasi
              </Link>
            ...
            <Link
              ...
              className={
                menuOpen
                  ? "text-black"
                  : "text-white" + navLinkClass(isLinkActive(paths.donations))
              }
            >
              Lihat Donasi
            </Link>
            <Link
              ...
              className={
                menuOpen
                  ? "text-black"
                  : "text-white" + navLinkClass(isLinkActive(paths.artikel))
              }
            >
              Artikel
            </Link>
            {!user && (
              <div className="flex items-center justify-center gap-2 940:mt-0 940:hidden">
                <Link onClick={() => setMenuOpen(false)} to={paths.login}>
                  <button className="border-primary border-2 rounded text-primary px-4 py-2">
                    Masuk
                  </button>
                </Link>
                <Link onClick={() => setMenuOpen(false)} to={paths.register}>
...
const ProfileButton = ({ user, handleLogout }) => {
  ...
  const hasValidProfilePhoto =
    detilUserLogin?.fotoProfil &&
    typeof detilUserLogin.fotoProfil === "string" &&
    detilUserLogin.fotoProfil.trim() !== "";
    ...
    <div className="relative inline-block text-left">
      <div className="flex items-center relative gap-2">
        ...
        <button
          ...
          className="rounded-full p-0 mr-4 bg-transparent overflow-hidden"
        >
          {hasValidProfilePhoto ? (
            <img
              src={detilUserLogin.fotoProfil}
              alt="profile"
              className="border border-gray-500 w-14 h-14 bg-transparent rounded-full object-cover"
            />
          ) : (
            <img
              src={personProfile}
              alt="profile"
              className="p-2 bg-gray-500 bg-opacity-75 w-12 h-12 rounded-full object-cover"
            />
          )}
        </button>
      </div>
      <div
        className={`absolute right-0 mt-3 w-56 bg-white rounded-xl shadow-md space-y-4 font-medium transform transition-all duration-200 origin-top ${
          menuOpen
