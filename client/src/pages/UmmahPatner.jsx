...
export default function UmmahPartner() {
    ...
    try {
      if (!currentSessionId) {
        const res = await axiosInstance.post("/chat", {
          userId: user._id,
          initialMessage: { role: message.role, content: message.content },
        });
  ...
  const callGeminiApi = async (conversationForGemini, schema = null) => {
    ...
    const apiKey = " "; // Ganti dengan API key gemini Anda
  ...
  return (
    <div className="flex flex-col md:flex-row md:h-[calc(100vh-8rem)] bg-gray-100 font-inter">
      <div className="w-full md:w-1/3 lg:w-1/4 bg-white p-6 shadow-lg flex flex-col overflow-y-auto rounded-lg md:rounded-r-none">
        <h2 className="text-2xl font-bold text-center text-[#F79319] mb-6">
          Ummah Partner{" "}
        </h2>
      ...
      <div className="flex-1 flex flex-col bg-white shadow-lg md:ml-4 rounded-lg md:rounded-l-none">
        <div className="sticky top-0 z-10 p-4 bg-[#316B6C] text-white text-center text-xl font-semibold rounded-t-lg shadow-md">
          Obrolan dengan Ummah Partner AI
        </div>
        <div
          ref={chatContainerRef}
          className="flex-1 p-6 overflow-y-auto space-y-4 bg-gray-50 h-[60vh]"
        >
        ...
        {currentMode === "chat" && (
          <form
            ...
            className=" sticky bottom-0 z-10 p-4 border-t border-gray-200 bg-white rounded-b-lg shadow-inner"
          >
            <div className="flex items-center">
              ...
              <button
                type="submit"
                className="bg-[#316B6C] text-white p-3 rounded-full hover:bg-[#2A5C5D] transition-colors duration-300 disabled:opacity-50 shadow-md"
                    ...
                    <li
                      ...
                      className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-all duration-200 ${
                        currentChatSessionId === session._id
                          ? "bg-[#E0F2F2] text-[#316B6C] font-semibold shadow-inner"
