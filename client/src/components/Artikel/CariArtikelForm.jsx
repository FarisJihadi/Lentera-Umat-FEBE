...
export default function CariArtikelForm({
  setArticles,
  loading,
  setLoading,
  setIsSearched,
  ...
  const isInitialMount = useRef(true); // Untuk melacak render pertama
    ...
    try {
      ...
      if (searchKeyword.trim()) {
        filtered = filtered.filter((item) =>
          item.judulArtikel.toLowerCase().includes(searchKeyword.toLowerCase())
        );
      }
  ...
  return (
    <div className="grid place-items-center mt-10 item-h-screen p-2 md:mt-16 relative">
      <form
        ...
        className="flex w-full max-w-4xl overflow-visible relative shadow-[0px_0px_6px_2px_rgba(0,0,0,0.1)] rounded-xl border-2 border-gray-200"
      >
        {/* Dropdown */}
        <div className="relative">
          <div
            className="bg-white flex justify-between items-center md:p-6 px-4 py-6 rounded-l-xl md:gap-2 gap-1 max-w-60 border-r-[1px] border-[#D9D9D9] cursor-pointer"
            onClick={() => setIsOpen(!isOpen)}
          >
            <span className="md:text-md text-sm  font-semibold md:block">
              {selectedSort}
            </span>
            <img
              src={arrowDown}
              alt="arrowDown"
              className={`w-2  md:block ${
                isOpen ? "transform rotate-180 w-2" : ""
              }`}
            />
          </div>
        ...
        <input
          type="text"
          className="flex-1 p-5 rounded-r-xl border-gray-300 focus:outline-none"
        ...
        <button
          type="submit"
          className="bg-primary py-2 px-4 rounded-md absolute right-5 top-3"
