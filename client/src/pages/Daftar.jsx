  ...
  const handleDaftar = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 409) {
        errorMessage =
          err.response.data.message || "Username atau email sudah terdaftar.";
      } else if (
        err.response &&
        err.response.data &&
    ...
    <div className="flex justify-between items-center mb-8">
      <div>
        <p className="md:text-heading-[36px] sm:text-3xl text-[28px] font-extrabold mb-2 mt-5">
          {role === "donatur" ? "Yuk" : "Yuk"}{" "}
          <span className="text-primary">
            {role === "donatur" ? "Daftar!" : "Daftar!"}
          </span>
        </p>
      </div>
  ...
  return (
    <>
      ...
      <div className="flex gap-4 mb-6 w-full max-w-md justify-center text-body-lg">
        <button
          className={`pb-2 md:text-normal text-[15px] font-medium border-0 rounded-none bg-white ${
            role === "donatur" ? "border-primary border-b-2" : ""
          } focus:outline-none`}
        ...
        <button
          className={`pb-2 md:text-normal text-[15px] font-medium border-0 rounded-none bg-white ${
            role === "komunitas" ? "border-primary border-b-2" : ""
          } focus:outline-none`}
          onClick={() => setRole("komunitas")}
        >
          Komunitas
        </button>
      </div>
