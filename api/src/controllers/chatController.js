  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          message:
            "userId dan initialMessage (dengan role dan content) diperlukan.",
        });
    }
    ...
    const newSession = new ChatSession({
      ...
      messages: [
        { role: initialMessage.role, content: initialMessage.content },
      ],
    });
    ...
    res
      .status(500)
      .json({ message: "Gagal membuat sesi chat.", error: error.message });
  }
};
...
exports.getChatSessionsByUser = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Gagal mengambil sesi chat.", error: error.message });
  }
};
...
exports.addMessageToSession = async (req, res) => {
  try {
    ...
    if (!newMessage || !newMessage.role || !newMessage.content) {
      return res
        .status(400)
        .json({ message: "newMessage dengan role dan content diperlukan." });
    }
    ...
    res
      .status(500)
      .json({
        message: "Gagal menambahkan pesan ke sesi.",
        error: error.message,
      });
  }
};
...
exports.deleteChatSession = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Gagal menghapus sesi chat.", error: error.message });
  }
};
