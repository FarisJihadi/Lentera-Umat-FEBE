...
function BookDetail() {
  ...
  if (error) {
    return (
      <div className="text-center py-20 bg-gray-50 min-h-screen">
        ...
        <Link
          to="/ummah-book"
          className="text-teal-600 hover:underline mt-4 inline-block px-4 py-2 border border-teal-600 rounded-md transition-colors hover:bg-teal-50"
        >
  ...
  const fileName = book.linkMateri
    ? getFileNameFromUrl(book.linkMateri)
    : "materi.pdf";

  const isOwner = user && book.materiUid === user._id;
  const secureLinkMateri = book.linkMateri
    ?.replace("http://", "https://")
        ...
        <div className="bg-[#E6F6F6] rounded-2xl p-5 md:p-8 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="md:col-span-1 flex justify-center items-start">
              <img
                src={
                  book.coverMateri?.replace("http://", "https://") ||
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex flex-wrap items-center gap-4 mt-4 mb-8">
                {book.linkMateri && (
                  <>
                    <a
                      href={secureLinkMateri}
                      download={`${book.judulMateri || "materi"}.pdf`}
                      className="cursor-pointer inline-flex items-center bg-orange-500 text-white px-4 py-2 rounded-lg font-semibold hover:bg-orange-600 transition-colors"
                    >
                      <svg
